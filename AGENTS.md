...
4. 你倾向把这个 vault 放在哪个目录？（默认 `~/linkc-os/`）
...
按 `03-schema/templates/journal.md` 模板，创建 `02-wiki/journal/<YYYY-MM-DD>.md`（`YYYY-MM-DD` 用真实当日日期，可执行 `date +%Y-%m-%d` 拿）。frontmatter 设 `entries: 1`。正文写一条 entry，记 "LinkcOS 初始化完成 + bootstrap by <你这个 agent 是什么>"。
...
- 如果用户用其他不支持 `CLAUDE_PROJECT_DIR` 的 Agent harness（Codex / Cursor 等），把这两处变量替换为用户在第 2 步给的实际 vault 绝对路径（比如 `/Users/<name>/linkc-os`）
