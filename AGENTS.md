...
4. 你想给你的个人 OS 起一个什么名字？
   - **默认 `myos`**
   - 也可以改成 `<你的名字>OS` / `Brain` / `第二大脑` / `LinkcOS`（保留模板原名）/ 任意你喜欢的
   - 这个名字会出现在 `CLAUDE.md`、session 简报、Skill 文件里——不影响 LICENSE 和原作者署名
5. 你想把这个仓库放在本机的哪个目录？
   - 默认 `~/myos`（和系统名对齐）
   - 如果你给系统起了别的名字，建议路径也对应改，比如 `~/brain`
...
### 第 7.5 步｜重命名系统（默认会执行——除非用户明确说要保留 "LinkcOS"）

第 2 步问题 4 的默认是 `myos`，所以**大多数 fork 用户都会触发重命名**。只有用户明确说"保留 LinkcOS 这个名字"时才跳过这一步。

在以下文件做整词替换 `LinkcOS` → `<用户选的名字>`（保持出现位置的语义一致；中文/英文标点周围都要兼容）：
...
如果用户明确选择了保留 "LinkcOS" 作为系统名，**跳过这一步**——什么都不做。
